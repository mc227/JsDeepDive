...
function doAdd() {
  ...
  var product = description.toUpperCase().big().bold().fontcolor('orange') + '<br/>' + 'Suggested by ' + email + '<br/>' + '£' + price + ' [projected sales ' + sales + ']<br/>'; // Append the current date/time to the product string.

  var ts = new Date();
  var tsStr = ts.getDate() + '/' + (ts.getMonth() + 1) + '/' + ts.getFullYear() + ', ' + pad(ts.getHours()) + ':' + pad(ts.getMinutes()) + ':' + pad(ts.getSeconds());
